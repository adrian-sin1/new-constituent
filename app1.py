...
# ------------------ PARSE & GROUP (Email + Subject) ------------------
reader = csv.DictReader(io.StringIO(uploaded_file.getvalue().decode("ISO-8859-1")))
grouped_threads = {}  # key = (email, subject), value = merged row
...
for row in reader:
    ...
    # Recover a real email if it's an Exchange DN embedded in the body
    ...
    # Parse the body into reply chunks (but DON'T add our own "From ..." label to avoid duplication)
    replies = extract_replies_with_senders(body, email)
    if replies:
        combined_reply_this_row = SEPARATOR.join(
            reply.strip() for _, reply in replies
        )
    else:
        combined_reply_this_row = body

    subj_key = subject if subject else "No Subject"
    key = (email.lower(), subj_key)  # group ONLY when Email + Subject match
    ...
    if key not in grouped_threads:
        ...
        # Merge this row's combined body with the existing thread using the same separator
        grouped_threads[key]["Reply"] += SEPARATOR + combined_reply_this_row

# ------------------ DATAFRAME & GROUPING FOR UI ------------------
df = pd.DataFrame(grouped_threads.values())
df["__order"] = range(len(df))
# For UI, group by Subject so subjects show together (emails are still distinct rows)
...
for i, (subject, group) in enumerate(grouped, start=1):
    ...
    # Count replies by counting separators in merged body
